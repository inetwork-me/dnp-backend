...
@section('content')
    <div class="card statistcs_card add_form m-0 mb-3">
        <div class="card-header">
            <span>{{ translate('Edit Field Group') }}</span>
        </div>
        <form class="form-horizontal w-100" action="{{ route('cms.acf.update', $fieldGroup->id) }}" method="POST">
            <div class="card-body">
                @csrf
                @method('PUT')
                <!-- Field Group Name -->
                <div class="form-group">
                    <input type="text" name="name" class="form-control" required
                        placeholder="{{ translate('Field Group Name') }}" value="{{ $fieldGroup->title }}">
                </div>

                <div class="form-group">
                    <textarea name="description" id="description" cols="30" rows="3" class="form-control">{{ $fieldGroup->description }}</textarea>
                </div>
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-md-4 mb-2">
                                        <input type="text" name="fields[{{ $index }}][label]"
                                            class="form-control" placeholder="Label" required value="{{ $custom->label }}">
                                    </div>
                                    <div class="col-md-4 mb-2">
                                        <input type="text" name="fields[{{ $index }}][name]" class="form-control"
                                            placeholder="Name" required value="{{ $custom->name }}">
                                    </div>
                                </div>

                                <div class="row align-items-center">
                                    <div class="col-md-4 mb-2 choices-textarea d-none">
                                        <textarea name="fields[{{ $index }}][choices]" class="form-control"
                                            placeholder="{{ translate('Enter choices separated by commas') . ' key=>value,key=>value' }}">{{ $custom->choices }}</textarea>
                                    </div>
                                    <div class="col-md-4 mb-2 default-value-group">
                                        <input type="text" name="fields[{{ $index }}][default]"
                                            class="form-control default-value-input" placeholder="Default value"
                                            value="{{ $custom->default_values }}">
                                        <textarea name="fields[{{ $index }}][default]" class="form-control default-value-textarea d-none"
                                            placeholder="Enter default values separated by commas">{{ $custom->default_values }}</textarea>
                                    </div>
                                    <div class="col-md-2 mb-2 select-multiple-toggle d-none">
                                        <div class="form-check form-switch">
                                            <label class="aiz-switch aiz-switch-success mb-0">
                                                <input type="checkbox" name="fields[{{ $index }}][multiple]"
                                                    id="multiple-toggle-0" value="1" id="validation-toggle-0"
                                                    @if ($custom->select_multiple) checked @endif>
                                                <span></span>
                                    ...
                                    <div class="col-md-2 mb-2">
                                        <div class="form-check form-switch">
                                            <label class="aiz-switch aiz-switch-success mb-0">
                                                <input type="checkbox" name="fields[{{ $index }}][validation]"
                                                    value="1" id="validation-toggle-0"
                                                    @if ($custom->is_required) checked @endif>
                                                <span></span>
                                                {{ translate('Required') }}
                                            </label>
                                        </div>
                    ...
                    @else
                        <div class="field-group border rounded p-3 mb-3 bg-white">
                            ...
                            <div class="row mb-2">
                                <div class="col-md-4 mb-2">
                                    <select name="fields[0][type]" class="form-control select2-with-icons">
                                        <option value="text" data-icon="fa-solid fa-i-cursor">{{ translate('Text') }}
                                        </option>
                                        <option value="password" data-icon="fa-solid fa-key">{{ translate('Password') }}
                                        </option>
                                        <option value="email" data-icon="fa-regular fa-envelope">
                                            {{ translate('Email') }}
                                        </option>
                                        <option value="number" data-icon="fa-solid fa-hashtag">{{ translate('Number') }}
                                        </option>
                                        <option value="tel" data-icon="fa-solid fa-phone">{{ translate('Phone') }}
                                        </option>
                                        <option value="url" data-icon="fa-solid fa-link">{{ translate('URL') }}
                                        </option>
                                        <option value="search" data-icon="fa-solid fa-magnifying-glass">
                                            {{ translate('Search') }}</option>
                                        <option value="date" data-icon="fa-regular fa-calendar-days">
                                            {{ translate('Date') }}
                                        </option>
                                        <option value="datetime-local" data-icon="fa-regular fa-clock">
                                            {{ translate('Datetime') }}</option>
                                        <option value="month" data-icon="fa-solid fa-calendar-alt">
                                            {{ translate('Month') }}
                                        </option>
                                        <option value="week" data-icon="fa-solid fa-calendar-week">
                                            {{ translate('Week') }}
                                        </option>
                                        <option value="time" data-icon="fa-regular fa-clock">{{ translate('Time') }}
                                        </option>
                                        <option value="color" data-icon="fa-solid fa-palette">{{ translate('Color') }}
                                        </option>
                                        <option value="file" data-icon="fa-solid fa-file-arrow-up">
                                            {{ translate('File') }}
                                        </option>
                                        <option value="checkbox" data-icon="fa-regular fa-square-check">
                                            {{ translate('Checkbox') }}</option>
                                        <option value="radio" data-icon="fa-regular fa-dot-circle">
                                            {{ translate('Radio') }}
                                        </option>
                                        <option value="range" data-icon="fa-solid fa-sliders-h">{{ translate('Range') }}
                                        </option>
                                        <option value="hidden" data-icon="fa-solid fa-eye-slash">
                                            {{ translate('Hidden') }}
                                        </option>
                                        <option value="select" data-icon="fa-solid fa-caret-down">
                                            {{ translate('Select') }}
                                        </option>
                                    </select>
                                </div>
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-4 mb-2 default-value-group">
                                    <input type="text" name="fields[0][default]"
                                        class="form-control default-value-input" placeholder="Default value">
                ...
                <button type="button" class="btn btn-primary btn-sm"
                    id="add-field">{{ translate('Add Field') }}</button>

                <!-- Location Rules -->
                <hr>
                <h5>{{ translate('Location Rules') }}</h5>
                <div id="rules-container">
                    @if ($fieldGroup->locationRules)
                        @foreach ($fieldGroup->locationRules as $index => $location)
                        <div class="rule-group mb-3">
                            <div class="form-row">
                                <div class="col mb-3">
                                    <select name="rules[0][object]" class="form-control">
                                        <option value="post_type" @if($location->key == 'post_type') selected @endif>{{ translate('Post Type') }}</option>
                                        <option value="post" @if($location->key == 'post') selected @endif>{{ translate('Post') }}</option>
                                        <option value="page" @if($location->key == 'page') selected @endif>{{ translate('Page') }}</option>
                                        <!-- Add more -->
                                    </select>
                                </div>
                                <div class="col mb-3">
                                    <select name="rules[0][operator]" class="form-control">
                                        <option value="==" @if($location->operator == '==') selected @endif>{{ translate('is equal to') }}</option>
                                        <option value="!=" @if($location->operator == '!=') selected @endif>{{ translate('is not equal to') }}</option>
                                    </select>
                                </div>
                                <div class="col mb-3">
                                    <select name="rules[0][value]" class="form-control">
                                        <!-- Dynamically populate based on object selection (JS required) -->
                                        <option value="example" @if($location->value == 'example') selected @endif>{{ translate('Example') }}</option>
                                    </select>
                                </div>
